...
%token IF FOR DO WHILE BREAK SWITCH CONTINUE RETURN CASE DEFAULT GOTO SIZEOF OR_CONST AND_CONST E_CONST NE_CONST LE_CONST GE_CONST G_CONST L_CONST LSHIFT_CONST MUL_EQ DIV_EQ ADD_EQ PER_EQ RS_EQ SUB_EQ LS_EQ AND_EQ XOR_EQ OR_EQ RSHIFT_CONST REL_CONST INC_CONST DEC_CONST ELSE HEADER
...
external_decl				: function_definition
							...
							| compound_stat
							;
function_definition			: type_spec declarator compound_stat {scope--;}	
							| declarator compound_stat {scope--;}
							;
...
lists						: decl lists
							...
							| stat
							;
selection_stat				: SWITCH {scope++;enc_lno[scope] = lno;}'(' expression ')' compound_stat {scope--;}
							;
iteration_stat				: FOR '(' decl  expression ';' expression ')' stat {scope--;}
							| FOR '(' decl  expression ';'	')' stat {scope--;}
							| FOR '(' decl  ';' expression ')' stat {scope--;}
							| FOR '(' decl  ';' ')' stat {scope--;}
							| FOR '(' expression ';' expression ';' expression ')' stat {scope--;}
							| FOR '(' expression ';' expression ';'	')' stat {scope--;}
							| FOR '(' expression ';' ';' expression ')' stat {scope--;}
							| FOR '(' expression ';' ';' ')' stat {scope--;}
							| FOR '(' ';' expression ';' expression ')' stat {scope--;}
							| FOR '(' ';' expression ';' ')' stat {scope--;}
							| FOR '(' ';' ';' expression ')' stat {scope--;}
							| FOR '(' ';' ';' ')' stat {scope--;}
