...
int search(char *variable, char *type, int local_scope, int enclosing_line_no, int type_dec)
{
	...
	for (int i=0;i<no_of_entries;i++)
	{
		if(strcmp(variable,s[i].variable_name)==0)
		{
			...
			{
				printf("Same variable as %s-%s-%s Scope %d Encl lno = %d\n", s[i].variable_name, s[i].value_of_variables, s[i].type, s[i].scope_of_variable, s[i].enclosing_line_no);
				flag2=1;
				local_k=i;
				present = 1;
			}
...
void printTable(){
	printf("----------------------SYMBOL TABLE----------------------\n\n");
	printf("Id\t\tVal\tType\tScope\tLno1\tLno2(enclosing)\n\n");
	for(int i=0;i<no_of_entries;i++){
		printf("%s\t\t%s\t%s\t%d\t%d\t%d\n", s[i].variable_name, s[i].value_of_variables, s[i].type, s[i].scope_of_variable, s[i].line_no, s[i].enclosing_line_no);
	}
}
